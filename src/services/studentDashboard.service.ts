...
class StudentDashboardService {
   async getStudentDashboard(studentUserId: string) {
      ...
      const totalStudentPaidAmount = learningCommitments.reduce(
         (sum: number, lc: any) => sum + (lc.studentPaidAmount || 0),
         0
      );
      ...
      const topCourses = learningCommitments
         ...
         .map((lc: any) => ({
            ...
            subject:
               translateSubject(lc.teachingRequest?.subject) ||
            ...
            progress: {
               ...
               percentage: lc.totalSessions
                  ? Math.round((lc.completedSessions / lc.totalSessions) * 100)
                  : 0,
            },
   ...
   private async getCurrentMonthStats(
      ...
      const commitmentIds = await this.getStudentLearningCommitmentIds(
         studentId
      );
   ...
   private async getNextSession(studentId: string) {
      ...
      return {
         ...
         tutorName:
            learningCommitment.tutor?.userId?.name || "Gia sư không xác định",
         subject:
            translateSubject(learningCommitment.teachingRequest?.subject) ||
            "Môn học không xác định",
   ...
   private async getRecentQuizSubmissions(
      studentUserId: string,
      limit: number = 5
   ) {
   ...
   private async getRecentTeachingRequests(
      studentId: string,
      limit: number = 5
   ) {
   ...
   private async getStudentLearningCommitmentIds(
      studentId: string
   ...
   private buildTimeline(
      ...
      sessions.forEach((session) => {
         ...
         timelineItems.push({
            type: "SESSION",
            title: `Buổi học với ${
               learningCommitment.tutor?.userId?.name || "Gia sư"
            }`,
            description: `Môn học: ${translateSubject(
      ...
      quizSubmissions.forEach((submission) => {
         timelineItems.push({
            ...
            description: `Bài kiểm tra: ${
               submission.quizId?.title || "Không rõ"
            }`,
            status: "completed",
      ...
      teachingRequests.forEach((request) => {
         timelineItems.push({
            ...
            description: `Trạng thái: ${translateTeachingRequestStatus(
               request.status
            )}`,
