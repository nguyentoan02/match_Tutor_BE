      ...
      if (Array.isArray(data.certifications)) {
         data.certifications.forEach((newCert, index) => {
            if (newCert._id) {
               ...
               const existingCert = tutor.certifications?.find(
      ...
      if (certificationFiles && certificationFiles.length > 0) {
         ...
         mapping.forEach((map) => {
            ...
            if (map.certId) {
               ...
               console.log(
                  `Looking for certId ${map.certId}:`,
                  cert ? "FOUND" : "NOT FOUND"
               );
            }
            ...
            switch (map.action) {
               ...
               case "remove":
                  if (
                     map.imageIndex !== undefined &&
                     cert.imageUrls[map.imageIndex] !== undefined
                  ) {
                     const removedUrl = cert.imageUrls[map.imageIndex];
                     cert.imageUrls.splice(map.imageIndex, 1);
                     console.log(
                        `Removed image from cert ${cert.name}: ${removedUrl}`
                     );
                  }
                  break;
            }
