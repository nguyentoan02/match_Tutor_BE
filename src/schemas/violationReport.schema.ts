...
export const createViolationReportSchema = z.object({
   body: z.object({
      ...
      reason: z
         ...
         .optional(),
      // evidenceFiles có thể là array of URLs (nếu đã upload trước) hoặc sẽ được upload qua multer
      // Với FormData, có thể là string (JSON stringified) hoặc không có (files upload qua multer)
      evidenceFiles: z
         .preprocess(
            (val) => {
               // Nếu là string (từ FormData), parse thành array
               if (typeof val === "string") {
                  try {
                     return JSON.parse(val);
                  } catch {
                     return [val]; // Nếu không parse được, coi như single URL string
                  }
               }
               return val;
            },
         ...
         .optional(),
      relatedTeachingRequestId: z
         .string()
         .regex(/^[0-9a-fA-F]{24}$/, "Invalid teaching request ID format")
         .optional(),
   }),
});
...
// Schema for getting student's own reports
