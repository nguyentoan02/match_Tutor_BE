...
class AdminSessionController {
   async listDisputes(req: Request, res: Response, next: NextFunction) {
      try {
         if (!req.user?._id) throw new UnauthorizedError("Authentication required");
         const status = (req.query?.status as "OPEN" | "RESOLVED" | undefined) || undefined;
         const result = await adminSessionService.listDisputes(status);
         new OK({ message: "Session disputes fetched", metadata: result }).send(res);
      } catch (err) {
         next(err);
      }
   }
   ...
   async getDispute(req: Request, res: Response, next: NextFunction) {
      try {
         ...
         new OK({ message: "Session dispute fetched", metadata: result }).send(res);
      } catch (err) {
         next(err);
      }
   }
   ...
   async resolve(req: Request, res: Response, next: NextFunction) {
      try {
         ...
         new OK({ message: "Session dispute resolved", metadata: result }).send(res);
      } catch (err) {
         next(err);
      }
   }
